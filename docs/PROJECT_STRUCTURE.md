...
4. **Модули общаются через зафиксированные контракты, а не через внутренности
   друг друга**. Пока контракт держится, любой может менять реализацию
   у себя.
...
настроена. Оставлен как референс структуры и контрактов.
...
**Скрипты.** Принцип тонкие обёртки выполнен только для `serve.py`;
