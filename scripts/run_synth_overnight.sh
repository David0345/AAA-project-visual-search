#!/bin/bash
# Ночной автономный прогон траектории «синтетика → дообучение → замер».
# Генерация Qwen-запросов -> train_synth -> fine-tune L-16 (свип lr) + gopt -> eval_full.
# Лог: experiments/synth_overnight.log. Результаты: experiments/eval_full_ledger.jsonl
set -uo pipefail
...
eval_ckpt() {  # $1=model_name $2=run_dir $3=out_name
  local ckpt; ckpt=$(ls -t "$2"/best_ep*_model_only.pt 2>/dev/null | head -1)
  if [ -z "$ckpt" ]; then log "НЕТ чекпойнта в $2 — пропускаю eval"; return 1; fi
...
log "=== STAGE 1: генерация Qwen-запросов ($N_ITEMS товаров) ==="
CUDA_VISIBLE_DEVICES=$GPU $PY scripts/gen_queries_qwen.py --full --limit $N_ITEMS \
  --batch-size 8 --out data/interim/queries_qwen.parquet 2>&1 | grep -E "item/s|сохранено|генерим" || { log "FATAL gen"; exit 1; }

log "=== STAGE 2: сборка train_synth.parquet ==="
...
log "=== STAGE 3: fine-tune L-16 на синтетике (свип lr) ==="
for LR in 1e-6 2e-6; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PY scripts/finetune_mini.py --model siglip2_l16_256 --device cuda --amp \
    ...
    --run-name l16_synth_lr${LR} --out-dir experiments/l16_synth_lr${LR} 2>&1 | grep -E "Epoch|MRR=|img/s" | tail -8
  eval_ckpt siglip2_l16_256 experiments/l16_synth_lr${LR} l16_synth_lr${LR}_50k
done

log "=== STAGE 4: fine-tune gopt (рабочая модель) на синтетике, lr=1e-6 ==="
CUDA_VISIBLE_DEVICES=$GPU $PY scripts/finetune_mini.py --model siglip2_gopt_256 --device cuda --amp \
  ...
  --train-parquet data/interim/train_synth.parquet --save-ckpt best \
  --run-name gopt_synth_lr1e-6 --out-dir experiments/gopt_synth_lr1e-6 2>&1 | grep -E "Epoch|MRR=|img/s|CUDA out|Error" | tail -8
eval_ckpt siglip2_gopt_256 experiments/gopt_synth_lr1e-6 gopt_synth_lr1e-6_50k

log "=== DONE. Сводка ledger: ==="
$PY scripts/show_results.py 2>/dev/null | tail -3
tail -6 experiments/eval_full_ledger.jsonl
log "SYNTH_OVERNIGHT_DONE"
