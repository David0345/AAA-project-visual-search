...
Папка содержит EDA-отчёты и результаты фильтрации полного манифеста (1,214,677 строк, 274,138 товаров) на множество релевантных изображений для обучения image-similarity / text-to-image модели.

## С чего начать читать

1. **[`eda_report.md`](eda_report.md)** — полный EDA-отчёт по требованиям: размеры, aspect, заполненность метаданных, категории, бренды (язык / распределение), дубликаты. Сводный текстовый документ.
2. **[`filtering_report.md`](filtering_report.md)** — что именно отсеяли и почему, с примерами phash-групп и значениями порогов.
3. **[`report.html`](report.html)** — интерактивные графики и галереи аномалий (открыть в браузере).
4. **[`numbers.json`](numbers.json)** — все сырые числа для машинного парсинга.
